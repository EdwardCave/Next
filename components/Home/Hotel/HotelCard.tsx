...
const HotelCard = ({ hotel }: Props) => {
  return (
    <div>
    <div className='relative h-[300px] w-full cursor-pointer group overflow-hidden rounded-lg'>
      {/* Add to fav button*/}
      <div className='absolute top-4 right-4 z-20 w-8 h-8 bg-white rounded-full text-black flex justify-center items-center flex-col'>
        ...
        {/*Content*/}
        <div>
            <h1 className='text-lg font-semibold mt-4 text-blue-950 hover:text-black cursor-pointer transition-all duration-200'>{hotel.name}</h1>
            ...
            <p className='text-gray-700 mt-3 font-medium'>
                Staring from <span className='text-blue-600 font-bold'>US ${hotel.price}</span>
            </p>
        </div>
    </div>
  )
